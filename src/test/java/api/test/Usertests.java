...
public class Usertests {
    ...
    public void testupdateUserbyName() {
    	...
    	logger.info("***************** USER INFO UPDATING**********************");
         ...
         Response res =
        		 UserEndPoints.updateUser(this.payload.getUserName(), payload);
         		res.then().log().body().statusCode(200);
         		Assert.assertEquals(res.getStatusCode(), 200);
         		
         	// checking data after update
         	
         		Response res1 = UserEndPoints.readUser(this.payload.getUserName());
         		res1.then().log().all();
    ...
    public void testDeleteByName() {
    	logger.info("***************** USER DELETING**********************");
    		Response res =
    				UserEndPoints.deleteUser(this.payload.getUserName());
    		res.then().log().all();
    		Assert.assertEquals(res.getStatusCode(), 200);
